  ...
  if (TensorInitInt::isTypeSupported(elmType)) {
    ...
    case TensorInitType::Quant: {
      ...
      if (!isScaleArgument) {
        ...
        InitKey keyIntScale(
            type, mlir::IntegerType::get(elmType.getContext(), 8), seed, true);
        tensorInitializers[keyIntScale] = intScaleInit;
      } else {
        initPtr = elmType.isF32() ? floatScaleInit : intScaleInit;
      }
      break;
    }
    default:
